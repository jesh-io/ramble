...
struct DictationView: View {
    ...
    private var providerPicker: some View {
        Menu {
            Toggle("Product analytics (local logging only)", isOn: Binding(
                get: { model.config.analyticsEnabled },
                set: { model.setAnalyticsEnabled($0) }))
            Text("Only counts, duration and provider categories. Never dictated content.")
            Divider()
