---
name: release
description: Cut a Talky release — diff since the last tag, propose the semver bump, write real release notes + changelog, then (after confirmation) commit, tag, publish the GitHub release, and watch CI attach the binaries. Use when the user says "release", "cut a release", "ship a version", "tag a version".
---

# /release — cut a Talky release

You are producing a **user-facing** release for the Talky macOS app. The
...
git diff "$LAST"..HEAD -- Sources TalkyPhone scripts .github   # read the actual changes
```
Read the diff for user-visible behavior: new features, changed defaults,
new config keys (`Sources/TalkyCore/Config.swift`), new CLI commands
(`Sources/TalkyCLI/main.swift`), Settings UI changes, safety/guard changes,
...
```
Under the install section always include: download `Talky-macOS.zip`, move
to `/Applications`, first-launch Gatekeeper step (right-click → Open or
`xattr -dr com.apple.quarantine /Applications/Talky.app`) unless builds are
notarized, and that updating re-prompts Accessibility until Developer ID
signing is configured. Mention `talky-cli-macOS.zip` and `SHA256SUMS.txt`.
...
git tag -a vX.Y.Z -m "Talky X.Y.Z"
git push origin main && git push origin vX.Y.Z
gh release create vX.Y.Z --title "Talky X.Y.Z" --notes-file <notes.md>
...
```
Confirm all three assets are attached (`Talky-macOS.zip`,
`talky-cli-macOS.zip`, `SHA256SUMS.txt`). If CI fails, fix and re-run
