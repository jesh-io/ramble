...
public final class DictationSession: @unchecked Sendable {
    ...
    private var analyticsStart: Date?
    ...
    public init(config: RambleConfig, transcriber: Transcriber? = nil) {
        self.config = config
        ProductAnalytics.configure(config)
        self.transcriber = transcriber ?? RambleKit.makeTranscriber(config: config)
    }
    ...
    public func start() async {
        ...
        Analytics.dictationStarted(source: .microphone, provider: selected.engine, model: selected.model)
        analyticsStart = Date()
        guard await MicCapture.requestPermission() else {
            Analytics.operationFailed(stage: .permission)
            finishAnalytics(.failed, count: 0)
        ...
        do {
            ...
            Analytics.operationFailed(stage: .capture)
            finishAnalytics(.failed, count: 0)
        }
    }
    ...
    public func stop() async {
        ...
        do {
            ...
            if !raw.isEmpty, config.recordings.historyEnabled {
            ...
            finishAnalytics(.success, count: raw.count)
        } catch {
            self.stream = nil
            eventTask?.cancel()
    ...
    public func skipCleanup() {
        Analytics.cleanupSkipped()
        cleanupTask?.cancel()
    }

    public func cancelRecording() {
        guard state == .recording else { return }
        finishAnalytics(.cancelled, count: 0)
        mic.stop()
        startupTask?.cancel()
        startupTask = nil
        stream?.cancel()
    ...
    private func finishAnalytics(_ outcome: Analytics.Outcome, count: Int) {
        guard let start = analyticsStart else { return }
        analyticsStart = nil
        let selected = ProviderRegistry.activeSTTProvider(config)
        Analytics.dictationCompleted(source: .microphone, outcome: outcome,
            durationSeconds: Date().timeIntervalSince(start), characterCount: count,
