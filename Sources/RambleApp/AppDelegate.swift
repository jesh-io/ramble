...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var config = RambleConfig.load()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        DictationHistory.prune(olderThanHours: config.recordings.historyRetentionHours)
        ...
        let cfg = config
        Task.detached {
            try? await RambleKit.makeDefaultTranscriber(config: cfg).prepare()
        }
    }

    /// Starts/stops the trackpad gesture recognizer per config. No-op in
    /// builds without the RambleGestures add-on.
    ...
    private func observeCLICommands() {
        ...
        let cases: [(String, () -> Void)] = [
            ("io.ramble.toggle", { [weak self] in self?.toggle() }),
            ("io.ramble.start", { [weak self] in self?.startDictation() }),
            ("io.ramble.stop", { [weak self] in self?.stopDictation() }),
            ("io.ramble.reload", { [weak self] in self?.reloadConfig() }),
        ]
    ...
    private func setIdleIcon() {
        ...
        let image = NSImage(systemSymbolName: "mic", accessibilityDescription: "Ramble")
    ...
    private func setRecordingIcon() {
        guard let button = statusItem.button else { return }
        let image = NSImage(systemSymbolName: "mic.fill", accessibilityDescription: "Ramble recording")
    ...
    private func setProcessingIcon() {
        guard let button = statusItem.button else { return }
        let image = NSImage(systemSymbolName: "waveform", accessibilityDescription: "Ramble processing")
        button.image = image?.withSymbolConfiguration(
    ...
    private func rebuildMenu() {
        ...
        menu.addItem(.separator())
        menu.addItem(item("Quit Ramble", #selector(quit), key: "q"))
    ...
    @objc private func addVocabularyAction() {
        ...
        guard alert.runModal() == .alertFirstButtonReturn,
              let entry = RambleConfig.vocabularyEntry(from: field.stringValue) else { return }
    ...
    @objc private func fixLastDictation() {
        ...
        alert.informativeText = "Correct the text; Ramble learns misheard names/terms from your edits and copies the fixed text."
        ...
        let cfg = config
        Task {
            let entries = (try? await RambleKit.extractVocabulary(
    ...
    @objc private func openSettings() {
        if settingsController == nil {
            ...
            store.canDeleteStoredData = { [weak self] in self?.session?.state == nil || self?.session?.state == .idle }
    ...
    @objc private func openConfig() {
        _ = RambleConfig.load() // ensure file exists
        NSWorkspace.shared.open(RambleConfig.fileURL)
    }
