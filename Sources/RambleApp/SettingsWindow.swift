...
/// Observable wrapper around RambleConfig: every mutation persists to disk
/// and notifies the app so hotkey/menu/state stay current.
@MainActor
final class ConfigStore: ObservableObject {
    @Published var config: RambleConfig {
        didSet {
            ProductAnalytics.configure(config)
            if oldValue.recordings.enabled != config.recordings.enabled { Analytics.featureChanged(.recordings, enabled: config.recordings.enabled) }
            if oldValue.recordings.historyEnabled != config.recordings.historyEnabled { Analytics.featureChanged(.history, enabled: config.recordings.historyEnabled) }
            if oldValue.cleanup.enabled != config.cleanup.enabled { Analytics.featureChanged(.cleanup, enabled: config.cleanup.enabled) }
            if oldValue.gesture.enabled != config.gesture.enabled { Analytics.featureChanged(.gestures, enabled: config.gesture.enabled) }
            do { try config.save(); saveError = nil }
            catch { saveError = error.localizedDescription }
            onChange?(config)
        }
    }
    var onChange: ((RambleConfig) -> Void)?
    var canDeleteStoredData: () -> Bool = { true }
    @Published var saveError: String?

    init(config: RambleConfig) {
        self.config = config
    }
}
    ...
    var body: some View {
        Form {
            ...
            Section("Product analytics") {
                Toggle("Allow anonymous product analytics", isOn: $store.config.analyticsEnabled)
                Text("Off by default. Logs provider/model categories, length and duration buckets, coarse time of day, and success/failure counts locally. Never includes dictated content, audio, vocabulary, or account names. No events are sent to a server in this build. Turning this off stops collection and clears pending delivery.")
            ...
            Section("Speech Recognition") {
                ...
                Text("Remote engines send audio and personal vocabulary to the selected provider. OpenAI, Mistral and Groq currently support batch transcription.")
                    .font(.caption).foregroundStyle(.secondary)
            }
            Section("Dictation") {
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("Remote cleanup sends transcript text and personal vocabulary to the provider, including during dictation when incremental cleanup is enabled. Choose a local model to keep text on this device.")
                    .font(.caption).foregroundStyle(.secondary)
    ...
    private func cardBody(_ index: Int, plugin: ProviderPlugin?) -> some View {
        ...
        return Grid(alignment: .leadingFirstTextBaseline, horizontalSpacing: 14, verticalSpacing: 12) {
            GridRow {
                ...
                TextField("account name", text: Binding(
                    ...
                    set: { newID in
                        ...
                        guard !newID.isEmpty, oldID != newID,
                              !store.config.accounts.contains(where: { $0.id == newID }) else { return }
                        do {
                            if let key = APIKeyStore.read("account/" + oldID) {
                                try APIKeyStore.save(key, id: "account/" + newID)
                                try APIKeyStore.save("", id: "account/" + oldID)
                            }
                            binding.wrappedValue.id = newID
                        } catch { store.saveError = error.localizedDescription }
                    }))
            ...
            if plugin?.apiKeyRequired ?? true {
                GridRow {
                    fieldLabel("Keychain")
                    AccountKeyField(accountID: binding.wrappedValue.id) { store.objectWillChange.send() }
                }
                GridRow {
                    fieldLabel("API key")
                    HStack(spacing: 8) {
            ...
            GridRow {
                ...
                Button(role: .destructive) {
                    withAnimation(.snappy) {
                        ...
                        do { try APIKeyStore.save("", id: "account/" + id) }
                        catch { store.saveError = error.localizedDescription; return }
        ...
        Group {
            if let plugin, plugin.apiKeyRequired {
                ...
                let present = (!env.isEmpty && ProcessInfo.processInfo.environment[env]?.isEmpty == false)
                    || account.apiKey?.isEmpty == false || APIKeyStore.read("account/" + account.id) != nil
...
private struct VocabularyTab: View {
    ...
    private func add() {
        guard let entry = RambleConfig.vocabularyEntry(from: newEntry) else { return }
...
private struct StorageTab: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Files") {
                Toggle("Save transcript history", isOn: $store.config.recordings.historyEnabled)
                TextField("Keep history for hours (0 = forever)", value: $store.config.recordings.historyRetentionHours, format: .number)
                Text("Retention is checked when Ramble launches and after dictation. Turning storage off stops future saves; existing data remains until deleted or pruned.")
                    .font(.caption).foregroundStyle(.secondary)
                Button("Delete all recordings and history", role: .destructive) { confirmDeletion = true }
                if let deletionError { Text(deletionError).foregroundStyle(.red) }
                LabeledContent("Config", value: RambleConfig.fileURL.path)
                ...
                Button("Reveal Config in Finder") {
                    NSWorkspace.shared.activateFileViewerSelecting([RambleConfig.fileURL])
                }
            }
        }
        .formStyle(.grouped)
        .confirmationDialog("Delete all saved audio and transcript history? Stop any active dictation first. This cannot be undone.", isPresented: $confirmDeletion) {
            Button("Delete recordings and history", role: .destructive) {
                guard store.canDeleteStoredData() else {
                    deletionError = "Finish or cancel the current dictation before deleting stored data."
                    return
                }
                do {
                    try RecordingStore.deleteAll()
                    try DictationHistory.clear()
                    deletionError = nil
...
private struct AccountKeyField: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            HStack {
                ...
                Button("Save") {
                    do {
                        try APIKeyStore.save(key, id: "account/" + accountID)
                        key = ""
                ...
                Button("Remove") {
                    do { try APIKeyStore.save("", id: "account/" + accountID); message = "Removed from Keychain"; onChange() }
                    catch { message = error.localizedDescription }
                }
            }
