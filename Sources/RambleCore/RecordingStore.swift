...
/// `~/Library/Application Support/Ramble/recordings/<timestamp>/`
...
/// retention window. Optional transcript history has separate retention.
public enum RecordingStore {
    public static var baseDir: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Ramble/recordings")
    }
    ...
    public static func newSessionDir(date: Date = Date()) throws -> URL {
        ...
        try PrivateFiles.directory(baseDir)
    ...
    public static func writeRecord(_ record: SessionRecord, to sessionDir: URL) {
        ...
        if let data = try? encoder.encode(record) {
            try? PrivateFiles.write(data, to: sessionDir.appendingPathComponent("transcript.json"))
        }
    }
    ...
    public static func deleteAll() throws {
        ...
        for child in try FileManager.default.contentsOfDirectory(at: baseDir, includingPropertiesForKeys: nil) {
