...
public struct OpenAICompatCleaner: TextCleaner {
    ...
    public func clean(_ text: String) async throws -> String {
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            ...
            throw RambleError("Cleanup provider '\(id)' returned HTTP \(status): \(snippet)")
        }
        guard
            ...
            throw RambleError("Cleanup provider '\(id)' returned an unexpected response shape")
        }
        ...
        guard !cleaned.isEmpty else {
            throw RambleError("Cleanup provider '\(id)' returned empty text")
        }
        return cleaned
    }
