...
public struct AnthropicCleaner: TextCleaner {
    ...
    public init(provider: CleanupProvider, systemPrompt: String, timeout: TimeInterval = 30) throws {
        guard var url = URL(string: provider.baseURL) else {
            throw RambleError("Invalid baseURL for provider '\(provider.id)': \(provider.baseURL)")
        }
    ...
    public func clean(_ text: String) async throws -> String {
        ...
        guard let apiKey else {
            throw RambleError("Anthropic account '\(id)' has no API key (set its apiKeyEnv variable)")
        }
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            ...
            throw RambleError("Anthropic '\(id)' returned HTTP \(status): \(snippet)")
        }
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw RambleError("Anthropic '\(id)' returned an unexpected response shape")
        }
        ...
        if let stopReason = json["stop_reason"] as? String, stopReason == "refusal" {
            throw RambleError("Anthropic '\(id)' declined the request (stop_reason: refusal)")
        }
        ...
        guard !cleaned.isEmpty else {
            throw RambleError("Anthropic '\(id)' returned empty text")
        }
        return cleaned
    }
}
