...
public struct AppleFMCleaner: TextCleaner {
    ...
    public func clean(_ text: String) async throws -> String {
        ...
        case .unavailable(let reason):
            throw RambleError("Apple on-device model unavailable: \(String(describing: reason)). Is Apple Intelligence enabled?")
        }
        ...
        guard !cleaned.isEmpty else {
            throw RambleError("Apple on-device model returned empty text")
        }
        return cleaned
        #else
        throw RambleError("FoundationModels framework not available on this OS")
        #endif
    }
}
