...
public final class AppleTranscriber: Transcriber, @unchecked Sendable {
    ...
    public func prepare() async throws {
        ...
        guard supported.contains(where: {
            ...
            throw RambleError("Locale '\(locale.identifier)' is not supported by Apple speech recognition")
        }
    ...
    public func makeStream(inputFormat: AVAudioFormat) async throws -> TranscriptionStream {
        ...
        guard let analyzerFormat = await SpeechAnalyzer.bestAvailableAudioFormat(compatibleWith: [transcriber]) else {
            throw RambleError("No compatible audio format for speech analysis")
        }
    ...
    init(
        ...
        if inputFormat == analyzerFormat {
            ...
            guard let converter = AVAudioConverter(from: inputFormat, to: analyzerFormat) else {
                throw RambleError("Cannot convert \(inputFormat) to \(analyzerFormat)")
            }
