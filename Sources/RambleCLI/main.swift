...
let usage = """
ramble — local voice-to-text

USAGE:
  ramble <media-file> [--raw] [--json] [--copy]   Transcribe an audio/video file
  ramble toggle | start | stop                    Control the Ramble menu bar app
  ramble clean [text]                             Clean text (arg or stdin) with the active model
  ramble history [n]                              Show last n dictations, raw vs cleaned (default 3)
  ramble audit [n]                                Diff-audit cleanup of last n dictations for hallucinations
  ramble guard <raw-file> <cleaned-file>          Run the hallucination guard on any pair
  ramble learn "<Term> = <misheard1>, <m2>"       Add a vocabulary term (= part optional)
  ramble vocab                                    List vocabulary entries
  ramble eval [provider-id]                       Score cleanup against your corrected dictations
  ramble usage                                    Usage & cost per day per model
  ramble models                                   List cleanup + speech providers
  ramble use <provider-id>                        Set the active cleanup provider
  ramble use --stt <provider-id> [--mode auto|streaming|batch]   Set the speech engine
  ramble download                                 Pre-download the on-device speech model
  ramble config                                   Print the config file path
...
FILE OPTIONS:
  ...
  --stt <id>  Use a specific speech engine for this file (see `ramble models`)

Config: ~/.config/ramble/config.json
"""
...
func postAppCommand(_ command: String) {
    DistributedNotificationCenter.default().postNotificationName(
        Notification.Name("io.ramble.\(command)"),
        object: nil,
        userInfo: nil,
...
case "config":
    print(RambleConfig.fileURL.path)
    _ = RambleConfig.load()
...
case "learn":
    guard args.count > 1 else { fail("usage: ramble learn \"Term = misheard1, misheard2\"") }
    guard let entry = RambleConfig.vocabularyEntry(from: args.dropFirst().joined(separator: " ")) else {
        fail("Could not parse vocabulary entry")
    }
    var config = RambleConfig.load()
    config.addVocabulary([entry])
    do {
        try config.save()
        print("learned: \(entry)")
        DistributedNotificationCenter.default().postNotificationName(
            Notification.Name("io.ramble.reload"), object: nil, userInfo: nil, deliverImmediately: true)
    } catch {
        fail("Could not save config: \(error.localizedDescription)")
    }

case "vocab":
    for entry in RambleConfig.load().vocabulary {
        print("- \(entry)")
    }
...
case "eval":
    ...
    Task {
        ...
        for session in goldenSessions {
            ...
            let output = (try? await RambleKit.cleanText(record.raw, config: cfg)) ?? record.raw
...
case "use":
    guard args.count > 1 else { fail("usage: ramble use <provider-id> | ramble use --stt <provider-id> [--mode auto|streaming|batch]") }
    var config = RambleConfig.load()
    if args[1] == "--stt" {
        guard args.count > 2 else { fail("usage: ramble use --stt <provider-id>") }
        let sttID = args[2]
        guard STTPlugins.availableProviders(config).contains(where: { $0.id == sttID }) else {
            fail("Unknown speech provider '\(sttID)'. Run `ramble models`.")
        }
    ...
    guard ProviderRegistry.allCleanupProviders(config).contains(where: { $0.id == id }) else {
        fail("Unknown provider '\(id)'. Run `ramble models` to list, or add it to \(RambleConfig.fileURL.path)")
    }
...
case "download":
    ...
    Task {
        do {
            ...
            try await RambleKit.makeDefaultTranscriber(config: config).prepare()
    ...
    Task {
        do {
            if args.contains("--incremental") {
                ...
                let cleaned = try await RambleKit.cleanText(input, config: config)
                print(cleaned)
            }
        } catch {
